...
class BotHubApp {
    ...
    const setupGsi = () => {
      if (window.google && window.google.accounts && window.google.accounts.id) {
        try {
          ...
          const container = document.getElementById('googleGsiContainer');
          if (container) {
  ...
  async handleGoogleLogin() {
    ...
    if (btn) {
      ...
      btn.innerHTML = `
        ...
        <div class="gag-text-wrap">
          ...
          <span class="gag-sub-text">Awaiting OAuth 2.0 Authorization</span>
        </div>
    ...
    const resetButton = () => {
      ...
      btn.innerHTML = `
        ...
        <div class="gag-text-wrap">
          ...
          <span class="gag-sub-text" id="txtGoogleHeroSub">Official Google OAuth 2.0</span>
        </div>
        <span class="gag-badge">OAUTH 2.0</span>
      `;
    };

    // 1. Trigger Google Identity Services One Tap / Prompt
    if (window.google && window.google.accounts && window.google.accounts.id) {
      try {
        window.google.accounts.id.prompt((notification) => {
          if (notification.isNotDisplayed()) {
            console.warn('Google prompt not displayed:', notification.getNotDisplayedReason());
            this.tryGoogleOAuthTokenClient(resetButton);
          } else if (notification.isSkippedMoment() || notification.isDismissedMoment()) {
            console.log('Google prompt dismissed by user');
            resetButton();
    ...
    // 2. Trigger Google OAuth 2.0 Token Client
    this.tryGoogleOAuthTokenClient(resetButton);
  }

  tryGoogleOAuthTokenClient(resetButton) {
      ...
      try {
        const tokenClient = window.google.accounts.oauth2.initTokenClient({
          ...
          error_callback: (err) => {
            ...
            this.showToast('Google OAuth window closed or origin not authorized in Google Cloud Console.', 'warning');
          }
        });
        tokenClient.requestAccessToken({ prompt: 'select_account' });
      } catch (err) {
        console.warn('Token client init error:', err);
        resetButton();
        this.showToast('Google OAuth SDK initialization error.', 'danger');
      }
    } else {
      resetButton();
      this.showToast('Google OAuth SDK is loading... please try again.', 'info');
    }
  }
