/**
 * Lightweight QR Code Generator (Pure JavaScript, Zero Dependencies)
 * Generates ISO/IEC 18004 compliant QR Codes for Crypto Addresses
 */
(function() {
  ...
  // Global helper to render QR code into canvas
  window.QRCodeGenerator = {
    renderCanvas: function(canvas, text, options) {
      ...
      // Dark modules
      ctx.fillStyle = options.foreground || '#0b0e14';
      ...
      if (options.showBadge !== false) {
        ...
        ctx.save();
        ctx.beginPath();
        ctx.arc(centerX, centerY, badgeRadius + 2, 0, 2 * Math.PI, false);
