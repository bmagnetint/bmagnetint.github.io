document.addEventListener('DOMContentLoaded', () => {
  ...
  // Daily profit: $10 - $30 / day per bot (Mon-Fri trading, Sat/Sun Off)
  const minDaily = botCount * 10;
  const maxDaily = botCount * 30;

  // Trading Days: ~22 days/mo
  const minMonthly = minDaily * 22;
  const maxMonthly = maxDaily * 22;

  const totalOutlay = capital + totalRent;
  const minReturn = minMonthly * monthsParam;
  const maxReturn = maxMonthly * monthsParam;

  if (detailTitle) detailTitle.textContent = titleStr;
  if (detailMonthlyHero) detailMonthlyHero.innerHTML = `$${minMonthly.toLocaleString()} – $${maxMonthly.toLocaleString()} <span style="font-size: 1rem; color: var(--text-muted);">/ month</span>`;
  if (detailDailyHero) detailDailyHero.textContent = `$${minDaily.toLocaleString()} – $${maxDaily.toLocaleString()} / day`;
  ...
  if (tblDaily) tblDaily.textContent = `$${minDaily.toLocaleString()} – $${maxDaily.toLocaleString()} / day (Mon–Fri)`;
  if (tblReturn) tblReturn.textContent = `+$${minReturn.toLocaleString()} – $${maxReturn.toLocaleString()}`;
